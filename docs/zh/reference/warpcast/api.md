...
分页端点会在 `result` 对象旁返回 `next.cursor` 属性。要获取下一页数据，请将该值作为 `cursor` 查询参数发送。可选参数 `limit` 可用于指定每页大小。
...
- 频道：Warpcast 基于 FIP-2（在 casts 上设置 `parentUrl`）构建了频道概念。你可以在[文档](https://www.notion.so/warpcast/Channels-4f249d22575348a5a0488b5d86f0dd1c?pvs=4)中了解更多关于频道的信息。
