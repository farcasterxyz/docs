...
当您的应用能够识别并认证用户后，您可以向他们展示通过 Warpcast 连接的选项。
...
除了生成新的密钥对外，您的应用还必须使用其 FID 的托管地址生成 ECDSA 签名。这允许将密钥归属于应用，对于了解正在使用的应用或基于生成内容的应用程序过滤内容等多种用途非常有用。
...
此值控制签名密钥请求签名的有效期限。它是 Unix 时间戳（秒）（注意：JavaScript 使用毫秒）。建议设置为 24 小时。

### 3. 应用使用公钥 + SignedKeyRequest 签名通过 Warpcast API 发起签名密钥请求
...
如果此请求来自可以打开深度链接的原生移动应用，您可以包含一个 redirectUrl，用户在完成请求后应被带回该 URL。

注意：如果您的应用是 PWA 或 Web 应用，请不要包含此值，因为用户将被带到没有状态的会话。
...
应用呈现深度链接，将提示用户打开 Warpcast 应用并授权签名者请求（底部有截图）。应用应指导用户在他们安装了 Warpcast 的移动设备上打开链接：

1. 在移动设备上时，直接触发深度链接
2. 在 Web 上时，将深度链接显示为二维码供扫描
...
向用户呈现深度链接后，应用必须等待用户完成签名者请求流程。应用可以轮询签名者请求资源并查找表明用户已完成请求的数据：
...
应用可以赞助签名者，这样用户无需支付费用。应用必须在 Warpcast 上注册并拥有 ≥ 100 warps。
