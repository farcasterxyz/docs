...
以下表格创建于存储来自 Hubs 数据的 Postgres 数据库中：

## chain_events（链上事件表）

存储从 hub 事件流接收的所有链上事件。这些事件代表任何链上操作，包括注册、转移、签名者添加/移除、存储租金支付等。事件永远不会被删除（即该表仅追加数据）。

| 列名              | 数据类型                   | 描述                                                                                  |
| ----------------- | -------------------------- | ------------------------------------------------------------------------------------- |
| id                | `uuid`                     | 该数据库特有的通用标识符（又称[代理键](https://en.wikipedia.org/wiki/Surrogate_key)） |
| created_at        | `timestamp with time zone` | 该行在数据库中首次创建的时间（与消息时间戳不同！）                                    |
| block_timestamp   | `timestamp with time zone` | 事件所在区块的 UTC 时间戳                                                             |
...
| transaction_hash  | `bytea`                    | 触发该事件的交易哈希                                                                  |
| body              | `json`                     | 链事件主体的 JSON 表示（根据 `type` 不同而变化）                                      |
| raw               | `bytea`                    | 序列化的 `OnChainEvent` [protobuf](https://protobuf.dev/) 原始字节                    |
...
| 列名             | 数据类型                   | 描述                                             |
| ---------------- | -------------------------- | ------------------------------------------------ |
| fid              | `bigint`                   | 用户的 FID（主键）                               |
| created_at       | `timestamp with time zone` | 该行在数据库中首次创建的时间（与注册时间不同！） |
...
| custody_address  | `bytea`                    | 拥有该 FID 的托管地址                            |
...
存储所有已观测到的用户名证明。包括已失效的证明（通过 `deleted_at` 列软删除）。查询用户名时，建议直接查询 `fnames` 表而非此表。
...
存储当前注册的所有用户名。如果用户名被注销，该行会通过 `deleted_at` 列软删除，直到为给定 FID 注册新用户名。
...
存储从 hub 获取的所有 Farcaster 消息。消息永远不会被删除，只会被软删除（即标记为删除但实际不从数据库中移除）。
...
| created_at       | `timestamp with time zone` | 该行在数据库中首次创建的时间（与消息时间戳不同！）                                    |
| updated_at       | `timestamp with time zone` | 该行最后更新时间                                                                      |
| timestamp        | `timestamp with time zone` | 消息的 UTC 时间戳                                                                     |
| deleted_at       | `timestamp with time zone` | hub 认为消息被删除的时间（例如响应 `CastRemove` 消息等）                              |
...
| raw              | `bytea`                    | 序列化消息的 [protobuf](https://protobuf.dev/) 原始字节                               |
...
| updated_at         | `timestamp with time zone` | 该行最后更新时间                                                                      |
| timestamp          | `timestamp with time zone` | 消息的 UTC 时间戳                                                                     |
| deleted_at         | `timestamp with time zone` | hub 认为 cast 被删除/撤销/清理的时间（例如响应 `CastRemove` 消息等）                  |
| fid                | `bigint`                   | 签署消息的用户 FID                                                                    |
| parent_fid         | `bigint`                   | 如果此 cast 是回复，父 cast 作者的 FID。否则为 `null`                                 |
| hash               | `bytea`                    | 消息哈希                                                                              |
| root_parent_hash   | `bytea`                    | 如果此 cast 是回复，回复链中原始 cast 的哈希。否则为 `null`                           |
| parent_hash        | `bytea`                    | 如果此 cast 是回复，父 cast 的哈希。否则为 `null`                                     |
| root_parent_url    | `text`                     | 如果此 cast 是回复，回复链中原始 cast 所回复的 URL                                    |
| parent_url         | `text`                     | 如果此 cast 是回复 URL（例如 NFT、网页 URL 等），则为该 URL。否则为 `null`            |
| text               | `text`                     | 去除提及内容的 cast 原始文本                                                          |
| embeds             | `json`                     | 与此 cast 一起嵌入的 URL 或 cast ID 数组                                              |
| mentions           | `json`                     | cast 中提及的 FID 数组                                                                |
| mentions_positions | `json`                     | cast 中被提及 FID 的 UTF8 字节偏移量                                                  |
...
| updated_at       | `timestamp with time zone` | 该行最后更新时间                                                                      |
| timestamp        | `timestamp with time zone` | 消息的 UTC 时间戳                                                                     |
| deleted_at       | `timestamp with time zone` | hub 认为反应被删除的时间（例如响应 `ReactionRemove` 消息等）                          |
| fid              | `bigint`                   | 签署消息的用户 FID                                                                    |
| target_cast_fid  | `bigint`                   | 如果目标是 cast，cast 作者的 FID。否则为 `null`                                       |
| type             | `smallint`                 | 反应类型                                                                              |
| hash             | `bytea`                    | 消息哈希                                                                              |
| target_cast_hash | `bytea`                    | 如果目标是 cast，cast 的哈希。否则为 `null`                                           |
...
| ----------------- | -------------------------- | ------------------------------------------------------------------------------------- |
| id                | `uuid`                     | 该数据库特有的通用标识符（又称[代理键](https://en.wikipedia.org/wiki/Surrogate_key)） |
| created_at        | `timestamp with time zone` | 该行在数据库中首次创建的时间（非链接在网络上创建的时间！）                            |
| updated_at        | `timestamp with time zone` | 该行最后更新时间                                                                      |
| timestamp         | `timestamp with time zone` | 消息的 UTC 时间戳                                                                     |
| deleted_at        | `timestamp with time zone` | hub 认为链接被删除的时间（例如响应 `LinkRemoveMessage` 消息等）                       |
...
| updated_at     | `timestamp with time zone` | 该行最后更新时间                                                                      |
| timestamp      | `timestamp with time zone` | 消息的 UTC 时间戳                                                                     |
| deleted_at     | `timestamp with time zone` | hub 认为验证被删除的时间（例如响应 `VerificationRemove` 消息等）                      |
| fid            | `bigint`                   | 签署消息的用户 FID                                                                    |
| hash           | `bytea`                    | 消息哈希                                                                              |
| signer_address | `bytea`                    | 被验证的钱包地址                                                                      |
| block_hash     | `bytea`                    | 验证所有权时的最新区块哈希                                                            |
...
| updated_at | `timestamp with time zone` | 该行最后更新时间                                                                      |
| timestamp  | `timestamp with time zone` | 消息的 UTC 时间戳                                                                     |
| deleted_at | `timestamp with time zone` | hub 认为数据被删除的时间                                                              |
| fid        | `bigint`                   | 签署消息的用户 FID                                                                    |
...
存储每个 FID 购买了多少存储单元以及过期时间。

| 列名           | 数据类型                   | 描述                                                                                  |
| -------------- | -------------------------- | ------------------------------------------------------------------------------------- |
| id             | `uuid`                     | 该数据库特有的通用标识符（又称[代理键](https://en.wikipedia.org/wiki/Surrogate_key)） |
| created_at     | `timestamp with time zone` | 该行在数据库中首次创建的时间                                                          |
| updated_at     | `timestamp with time zone` | 该行最后更新时间                                                                      |
| rented_at      | `timestamp with time zone` | 消息的 UTC 时间戳                                                                     |
| expires_at     | `timestamp with time zone` | 此存储分配将过期的时间                                                                |
| chain_event_id | `uuid`                     | `chain_events` 表中表示分配存储的链上事件行的 ID                                      |
