...
ID 注册表记录每个 fid 与哪个以太坊地址关联，并管理 fid 的转移和恢复。

如果您想读取 fid 信息、转移或恢复 fid，或管理 fid 的恢复地址，请使用 ID 注册表。

如果您想注册新的 fid，请改用 [ID 网关](/reference/contracts/reference/id-gateway.md)。

## 读取

### idOf

返回地址拥有的 fid（`uint256`），如果地址不拥有 fid 则返回零。
...
返回当前已注册的最高 fid（`uint256`）。

### verifyFidSignature

检查消息是否由 fid 的当前托管地址签名。返回 `bool`。

| 参数           | 类型      | 描述             |
| -------------- | --------- | ---------------- |
| custodyAddress | `address` | 要验证签名的地址 |
...
如果直接调用会回滚。必须通过 [ID 网关](/reference/contracts/reference/id-gateway.md) 调用。
...
| 参数     | 类型      | 描述                                                            |
| -------- | --------- | --------------------------------------------------------------- |
| to       | `address` | 要转移 fid 的目标地址                                           |
| deadline | `uint256` | 签名截止时间                                                    |
| sig      | `bytes`   | 来自 `to` 地址的 EIP-712 [`Transfer`](#transfer-signature) 签名 |

::: warning
转移 fid 不会重置其恢复地址。要转移 fid 并更新其恢复地址，请调用 [`transferAndChangeRecovery`](#transferandchangerecovery)。如果从不信任的发送方接收 fid，请确保在转移时清除或更改其恢复地址。
:::

#### 转移签名

要将 fid 转移到另一个账户，调用者必须提供接收地址的 EIP-712 类型签名，格式如下：

`Transfer(uint256 fid,address to,uint256 nonce,uint256 deadline)`

| 参数     | 类型      | 描述                   |
| -------- | --------- | ---------------------- |
| fid      | `uint256` | 被转移的 fid           |
...
要将 fid 转移到另一个账户并更改恢复地址，必须提供 `to` 地址的 EIP-712 类型签名，格式如下：
...
要代表 fid 所有者更改恢复地址，调用者必须提供 `owner` 地址的 EIP-712 类型签名，格式如下：
...
使用 fid 恢复地址的签名将 fid 转移到新地址。调用者必须提供两个 EIP-712 [`Transfer`](#transfer-signature) 签名：一个来自恢复地址授权转出，一个来自 `to` 地址接受转入。这些消息具有[相同格式](#transfer-signature)。
