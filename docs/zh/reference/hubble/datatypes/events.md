...
当 Hubble 观察到状态变更时会发出事件。由于不同 Hub 可能以不同顺序接收消息，事件的顺序对每个 Hub 是特定的。客户端可以通过[事件 API](/reference/hubble/grpcapi/events)订阅 Hub，获取实时变更流。

Hubble 会保留事件 3 天，之后为节省空间将删除。如需获取更早的数据，请使用[GRPC](../grpcapi/grpcapi.md)或[HTTP](../httpapi/httpapi.md) API。
...
| 字段        | 类型        | 标签 | 描述                       |
| ----------- | ----------- | ---- | -------------------------- |
| migrated_at | [uint32](#) |      | Hub 迁移至 OP 主网的时间戳 |
