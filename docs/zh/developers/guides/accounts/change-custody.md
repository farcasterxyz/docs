...
账户由托管地址所有，该地址是 OP 主网上的一个以太坊地址。

用户可能出于安全原因或需要转移整个账户所有权而更改此地址。

### 要求

- 一个在 OP 主网上拥有该账户的 ETH 钱包，且钱包中有少量 ETH。
- 一个 OP 主网的以太坊提供商 URL（例如通过 [Alchemy](https://www.alchemy.com/)、[Infura](https://www.infura.io/) 或 [QuickNode](https://www.quicknode.com/) 获取）。

### 更改托管地址

调用 Id Registry 合约的 `transfer` 函数。接收地址必须提供 EIP-712 签名以接受转移。
...
请调用 [`transferAndChangeRecovery`](/reference/contracts/reference/id-registry#transferandchangerecovery)。
:::
